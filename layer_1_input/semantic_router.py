...
class SemanticRouter:
    ...
    def __init__(self):
        self._model = None
        # _intent_embeddings agora armazena uma matriz 2D (num_examples x embedding_dim)
        # para cálculos vetorizados ultra-rápidos com np.dot
            ...
            if intent_examples is None:
                ...
                if db_data:
                    ...
                    for intent_name, items in db_data.items():
                        ...
                        if embeddings_list:
                            self._intent_embeddings[intent_name] = np.vstack(embeddings_list)
            ...
            if not db_intents_loaded:
                ...
                if all_examples:
                    ...
                    for i, intent_name in enumerate(all_intents):
                        ...
                        # Normaliza a matriz em batch
                        norms = np.linalg.norm(intent_emb, axis=1, keepdims=True)
                        self._intent_embeddings[intent_name] = intent_emb / norms
                        idx += count
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        for intent_name, embeddings_matrix in self._intent_embeddings.items():
            # Optimização: Multiplicação de matrizes (N x D) * (D,) = (N,)
            # ~10x mais rápido que list comprehensions O(N) para datasets grandes
            similarities = np.dot(embeddings_matrix, msg_embedding)

            # Top-3 para robustez (menos sensível a outliers)
            if len(similarities) >= 3:
                # np.partition é O(N) vs np.sort O(N log N)
