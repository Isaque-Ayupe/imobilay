...
class SemanticRouter:
    ...
    async def initialize(self, intent_examples: dict[str, list[str]] | None = None):
        ...
        try:
            ...
            # Cache the stacked matrices for faster vectorized dot products in route()
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        for intent_name, matrix in self._intent_embeddings_matrix.items():
            # ⚡ Bolt Optimization:
            # Replace list comprehension and O(N log N) `sorted()` with vectorized
            # matrix multiplication (`np.dot`) and O(N) `np.partition()`.
