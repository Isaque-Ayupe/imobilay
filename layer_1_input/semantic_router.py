...
class SemanticRouter:
    ...
    async def initialize(self, intent_examples: dict[str, list[str]] | None = None):
        ...
        try:
            ...
            # Precompute stacked matrices for fast vector math
            for intent, embs in self._intent_embeddings.items():
                if embs:
                    self._intent_embeddings_matrix[intent] = np.stack(embs)
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        for intent_name, emb_matrix in self._intent_embeddings_matrix.items():
            # Vectorized dot product against all intent embeddings
            similarities = np.dot(emb_matrix, msg_embedding)

            # Top-3 para robustez usando O(N) partition ao invés de O(N log N) sort
