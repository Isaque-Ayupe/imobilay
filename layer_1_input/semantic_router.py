...
class SemanticRouter:
    ...
    async def initialize(self, intent_examples: dict[str, list[str]] | None = None):
        ...
        try:
            ...
            # ⚡ Bolt Optimization:
            # Pre-stack embeddings into a single NumPy matrix per intent during initialization.
            # This enables vectorized matrix-vector multiplication in _embedding_route,
            # eliminating python loop overhead for dot products.
            for intent_name, emb_list in self._intent_embeddings.items():
                if emb_list:
                    self._intent_embeddings_matrix[intent_name] = np.stack(emb_list)
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        # ⚡ Bolt Optimization:
        # Replaced Python list comprehensions and individual np.dot calls with O(1) vectorized
        # matrix multiplication. Replaced O(N log N) sorted() with O(N) np.partition() for top-K extraction.
        for intent_name, matrix in self._intent_embeddings_matrix.items():
            ...
            raw_scores[intent_name] = float(np.sum(top_k) / k) if k > 0 else 0.0
