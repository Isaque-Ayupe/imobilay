...
class SemanticRouter:
    ...
    async def initialize(self, intent_examples: dict[str, list[str]] | None = None):
        ...
        try:
            ...
            self._intent_embeddings_matrix = {
                name: np.array(embs) for name, embs in self._intent_embeddings.items()
            }
            ...
            self._intent_embeddings_matrix = {
                name: np.array(embs) for name, embs in self._embeddings_cache.items()
            }
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        # ⚡ Bolt Optimization:
        # Use vectorized np.dot matrix multiplication instead of looping list comprehensions
        # and replace O(N log N) sorted() with O(N) np.partition()
