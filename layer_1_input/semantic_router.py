...
class SemanticRouter:
    ...
    async def initialize(self, intent_examples: dict[str, list[str]] | None = None):
        ...
        try:
            ...
            self._intent_embeddings_matrix = {
                k: np.array(v) for k, v in self._intent_embeddings.items()
            }
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        """Classificação via embedding cosine similarity (Otimizado)."""
        ...
        for intent_name, matrix in self._intent_embeddings_matrix.items():
            ...
            similarities = matrix.dot(msg_embedding)

            # Top-3 para robustez (menos sensível a outliers)
            # Use O(N) np.partition instead of O(N log N) sorted
