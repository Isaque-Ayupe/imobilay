            ...
            if intent_examples is None:
                ...
                if db_data:
                    ...
                    for intent_name, items in db_data.items():
                        ...
                        if embeddings_list:
                            self._intent_embeddings[intent_name] = np.array(embeddings_list)
            ...
            if not db_intents_loaded:
                ...
                if all_examples:
                    ...
                    for i, intent_name in enumerate(all_intents):
                        ...
                        normalized_embs = [emb / np.linalg.norm(emb) for emb in intent_emb]
                        if normalized_embs:
                            self._intent_embeddings[intent_name] = np.array(normalized_embs)
                        idx += count
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        for intent_name, embeddings_matrix in self._intent_embeddings.items():
            ...
            if len(similarities) > k:
                # O(N) top-K extraction instead of O(N log N) sorting
