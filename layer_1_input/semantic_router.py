...
class SemanticRouter:
            ...
            if intent_examples is None:
                ...
                if db_data:
                    ...
                    for intent_name, items in db_data.items():
                        ...
                        if embs:
                            self._intent_embeddings[intent_name] = np.vstack(embs)
            ...
            if not db_intents_loaded:
                ...
                if all_examples:
                    ...
                    for i, intent_name in enumerate(all_intents):
                        ...
                        if normalized_embs:
                            self._intent_embeddings[intent_name] = np.vstack(normalized_embs)
                        idx += count
    ...
    def _embedding_route(self, message: str) -> RoutingResult:
        ...
        # ⚡ Bolt Optimization: Compute cosine similarities simultaneously using NumPy
        # matrix-vector multiplication instead of Python list comprehensions to improve routing latency.
        for intent_name, embeddings_matrix in self._intent_embeddings.items():
            similarities = np.dot(embeddings_matrix, msg_embedding).tolist()
