...
async def list_sessions(user_id: str):
    ...
    try:
        ...
        # ⚡ Bolt: Cache current date outside the loop to prevent redundant system calls
        # and datetime object instantiations on every iteration.
        today_date = datetime.now().date()
        yesterday_date = today_date - timedelta(days=1)

        for s in sessions:
            session_date = s.last_active.date()
            is_today = session_date == today_date
            is_yesterday = session_date == yesterday_date
