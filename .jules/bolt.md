...
## 2026-03-28 - Loop Date Extraction
**Learning:** Calculating `datetime.now().date()` inside a loop over a large result set causes redundant system calls and objects (O(N) vs O(1)) and can lead to edge-case bugs if the iteration crosses the midnight boundary.
**Action:** Extract invariant operations like current date calculations outside of iterative processing loops.
