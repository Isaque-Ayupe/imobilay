...
## 2026-03-27 - Loop Invariant Date Calculation
**Learning:** In backend loops processing collections (e.g., sessions in `api.py`), calls to standard libraries like `datetime.now().date()` evaluate repeatedly per iteration. While individually fast, executing them N times inside a loop generates significant overhead (10x slower locally).
**Action:** Extract invariant function outputs, such as current date and standard timedelta calculations, outside of iterative loops to prevent redundant system calls and object instantiation.
