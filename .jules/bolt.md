...
## 2026-03-27 - Extracting Invariant Date Calculations from Loops
**Learning:** Performing `datetime.now().date()` inside an iterative loop is an O(N) operation that causes redundant system calls and introduces a risk of inconsistent state if the loop execution crosses midnight.
**Action:** Always extract invariant function outputs (like current date/time calculations) to a variable outside the loop to make it an O(1) operation.
