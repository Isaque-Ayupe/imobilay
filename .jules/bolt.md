...
## 2026-03-28 - Extract Invariant Dates in Loops
**Learning:** Calling `datetime.now().date()` inside a loop that iterates over a large number of items (like a list of sessions) causes redundant system calls and object instantiations on every iteration, leading to O(N) overhead.
**Action:** Always extract invariant function outputs, such as current date calculations, outside of iterative loops to prevent redundant overhead and improve performance.
