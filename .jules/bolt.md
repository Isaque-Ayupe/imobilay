...
## 2026-05-26 - Loop-Invariant Backend Operations
**Learning:** In backend endpoints processing lists (e.g., sessions), calling system-level functions like `datetime.now()` inside loops introduces unnecessary O(N) system call overhead, especially if the value remains logically constant for the duration of the request.
**Action:** Extract invariant function outputs, such as current date calculations, outside of iterative loops to prevent redundant system calls and improve request latency.
