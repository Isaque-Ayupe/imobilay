...
## 2026-03-27 - Sequential vs Concurrent IO in Scrapers
**Learning:** Sequential `await` calls for independent network requests (e.g., fetching from ZAP and VivaReal APIs one after the other) double the total network latency unnecessarily.
**Action:** Always wrap independent, IO-bound asynchronous tasks in `asyncio.gather()` to execute them concurrently and optimize total execution time.
