...
## 2024-06-29 - Pre-compute invariant mappings to optimize O(N log N) loops in parser
**Learning:** Dynamic generation of reference lists (like `city_variants` and flattening `KNOWN_NEIGHBORHOODS`) inside an iterative parser function causes severe performance degradation, specifically the repeated O(N log N) sorting overhead for `all_neighborhoods`.
**Action:** Extract invariant data mapping operations (e.g., dictionary lookups and list flattening/sorting) into module-level pre-computed constants to achieve O(1) loop setups and avoid redundant calculations on every execution.
