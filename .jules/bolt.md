...
## 2026-05-25 - [Optimize datetime calculation in api.py loops]
**Learning:** Redundant `datetime.now().date()` calls inside a loop iterate through all items on each pass, which adds significant overhead when dealing with numerous sessions. Extracting static loop boundaries before looping greatly improves efficiency without impacting data correctly.
**Action:** Always extract invariant function outputs (like current dates) that act as comparators outside of iterating loops, especially those executing in high-use endpoints.
