...
## 2026-05-22 - Semantic Router Cosine Similarity Optimization
**Learning:** In the backend, vectorizing cosine similarity calculations in `SemanticRouter` using NumPy matrix-vector multiplication (`np.dot` with pre-stacked embeddings per intent cached in `_intent_embeddings_matrix`) significantly improves performance over Python list comprehensions.
**Action:** Always replace Python list comprehension math with vectorized NumPy implementations for math-heavy code like vector similarity comparisons.
