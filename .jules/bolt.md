...
## 2026-03-28 - Loop Date Extraction Bottleneck and Edge Case
**Learning:** Evaluating `datetime.now().date()` repeatedly inside an iteration loop (like `sessions`) introduces O(N) redundant system calls. More importantly, it creates a subtle edge-case bug where the date could shift across a midnight boundary during execution, leading to inconsistent timestamps within the same response.
**Action:** Extract invariant function outputs, such as current date calculations, outside of iterative loops to pre-computed variables.
