...
## 2026-03-28 - Backend Date Calculation Bottleneck
**Learning:** Computing the current date and time (e.g., `datetime.now().date()`) inside a loop over a collection (like user sessions) introduces redundant O(N) system call overhead and risks edge-case bugs if the midnight boundary is crossed during iteration.
**Action:** Extract invariant function outputs, such as current date calculations, outside of iterative loops to reduce overhead to O(1).
