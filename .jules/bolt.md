...
## 2026-03-27 - Loop Bottlenecks with External Calls
**Learning:** Extract invariant function outputs, such as current date calculations (e.g., `datetime.now().date()`), outside of iterative loops to prevent redundant system calls and reduce overhead.
**Action:** Always pre-calculate loop-invariant values, especially system calls, before entering `for` loops to ensure scalability over large lists.
