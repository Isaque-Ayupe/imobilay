...
## 2026-06-14 - Concurrent API Fetching in WebScraperAgent
**Learning:** Sequential await calls for independent I/O-bound tasks in async Python functions (like fetching from two different APIs) artificially inflate execution time. The total time becomes the sum of both operations rather than the maximum.
**Action:** Always use `asyncio.gather` (or equivalent concurrency primitives) to run independent I/O-bound operations in parallel, effectively cutting latency down to the longest single operation.
