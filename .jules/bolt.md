...
## 2026-06-17 - Backend Date Calculation Optimization
**Learning:** Computing current dates inside iterative loops (e.g. `datetime.now().date()` inside a `for` loop processing database rows) creates significant overhead due to redundant system calls. For 100k iterations, it's roughly 35-40x slower than pre-computing the value.
