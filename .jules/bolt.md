...
## 2026-03-27 - Backend Static Processing Optimization
**Learning:** In string parsing functions like `parse_filters` in `execution/scraper_utils.py`, re-computing a list of known values (e.g. flattening and sorting `KNOWN_NEIGHBORHOODS`) on every invocation leads to repeated O(N log N) processing.
**Action:** Extract loop invariants for static constants to the module level so they are only processed once when the module loads, replacing repeated calculation overhead with an O(1) assignment.
