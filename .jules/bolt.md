...
## 2024-11-20 - Vectorized Cosine Similarity
**Learning:** Python list comprehensions and iterative `np.dot` calls for cosine similarities scale poorly as the number of intents/examples grows. O(N log N) sorting for top-K extraction is also sub-optimal.
**Action:** Use NumPy's `np.vstack` to precompute a single 2D matrix (`_all_embeddings_matrix`) and perform matrix-vector multiplication for simultaneous similarity calculations. Use `np.partition` instead of `sorted()` to achieve O(N) top-K element extraction.
