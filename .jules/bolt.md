...
## 2026-03-27 - Pre-computation of Constant Mappings and Lookups
**Learning:** In string parsing functions that iterate over mappings or flatten lists (like `parse_filters` building `all_neighborhoods` and defining `city_variants`), leaving these data transformations inside the function creates significant overhead (O(N log N) in the case of sorting). These recalculations happen on every function call, reducing throughput.
**Action:** Always extract invariant data processing operations—such as flattening static lists or building reference dictionaries—into module-level pre-computed constants to minimize per-execution overhead.
