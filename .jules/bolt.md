...
## 2025-04-30 - Vectorize SemanticRouter embedding similarities
**Learning:** In the backend, vectorizing cosine similarity calculations in `SemanticRouter` using NumPy matrix-vector multiplication (`np.dot` with a single 2D `_all_embeddings_matrix` stacked across all intents and an `_intent_indices` mask) significantly improves performance over Python list comprehensions and per-intent matrix multiplications.
**Action:** Use matrix operations and O(N) selection functions like `np.partition` instead of python loops and O(N log N) sorting for numerical operations.
