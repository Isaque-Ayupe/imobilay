...
## 2026-06-20 - Extract Invariant Data Processing Operations
**Learning:** In string parsing functions that are called frequently, dynamically calculating and sorting static reference lists (e.g., iterating through `KNOWN_NEIGHBORHOODS` dictionaries, creating flattened array elements, or generating invariant sets via O(N log N) `.sort()`) introduces repeated and redundant computational overhead per execution request.
**Action:** Extract invariant data processing operations, such as flattening arrays or sorting static reference lists, outside of request loops and parsing functions into module-level, pre-computed constants.
