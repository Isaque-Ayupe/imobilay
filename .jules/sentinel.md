...
## 2024-05-22 - IDOR Vulnerability in Sessions API
**Vulnerability:** The `/api/sessions` endpoint accepted a `user_id` parameter and used the singleton `get_system_client()` (which uses the service role key and bypasses RLS) to query the database. This allowed any unauthenticated or authenticated user to retrieve chat sessions for any arbitrary `user_id`.
**Learning:** Using system-level database clients in user-facing API routes completely circumvents the database's Row-Level Security policies, creating severe Insecure Direct Object Reference (IDOR) vulnerabilities.
**Prevention:** Always require an `Authorization` header containing a valid user JWT on endpoints that return user-specific data. Extract the JWT and pass it to a client instantiation function (like `get_user_client(jwt)`) that configures the connection with the user's context, ensuring that Supabase RLS policies are strictly enforced.
