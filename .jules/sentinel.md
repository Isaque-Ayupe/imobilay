...
## 2025-06-03 - Insecure Direct Object Reference (IDOR) via Bypassed Row-Level Security
**Vulnerability:** The `/api/sessions` endpoint used `get_system_client()` (with `service_role` key), completely bypassing Supabase Row-Level Security (RLS) and allowing any user to potentially view other users' sessions if user_ids were guessed or enumerated.
**Learning:** Returning user-specific data from database queries without passing the authenticated user's context (JWT) to the database client breaks the security boundary, relying solely on unverified API parameters instead of enforced database policies.
**Prevention:** Always enforce backend endpoints returning user-specific data to extract the user's JWT from the `Authorization` header and instantiate the Supabase client using `get_user_client(token)` to ensure RLS policies are applied at the database level.
