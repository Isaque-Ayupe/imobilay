...
## 2024-06-25 - API Insecure Direct Object Reference (IDOR)
**Vulnerability:** The /api/sessions endpoint used get_system_client() (service_role) and accepted any user_id, allowing users to read others' sessions.
**Learning:** Using system clients bypasses RLS policies and enables IDOR when returning user-specific data based on client-provided IDs.
**Prevention:** Extract the user JWT from the Authorization header and instantiate the database client using get_user_client(user_jwt) to properly enforce RLS policies.
