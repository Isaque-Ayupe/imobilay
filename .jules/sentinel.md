...
## 2024-07-04 - Fix Traceback Leakage to Standard Error
**Vulnerability:** Exception details and stack traces were leaked to standard error via `traceback.print_exc()` in API route handlers (`chat_endpoint` and `list_sessions`).
**Learning:** Printing stack traces to standard error bypasses the application's secure logging framework, increasing the risk of exposing internal system details and logic flaws if standard error is captured insecurely.
**Prevention:** Use `logger.exception()` to ensure tracebacks are securely processed and captured by the application's centralized logging framework.
