...
## 2025-03-10 - IDOR in Sessions Endpoint
**Vulnerability:** The `/api/sessions` endpoint fetched user sessions using `get_system_client()`, which bypasses RLS, based solely on a `user_id` query parameter, allowing anyone to fetch any user's sessions (IDOR).
**Learning:** System clients should never be used for frontend-facing data retrieval; endpoints must validate the user's JWT from the `Authorization` header to enforce database-level Row-Level Security (RLS).
**Prevention:** Always use `get_user_client(user_jwt)` and require a valid Bearer token for endpoints returning user-specific data.
